...
TermView = function() {
    this._options = {
      screen_size: {w: 80, h: 24},
      grid_size: {w: 20, h: 35},
      cjk_font: {face: ['HiraginoSansGB-W3', '"Microsoft JhengHei"', 'sans'], size: 24, offset: {x: -4, y: -2}},
      ascii_font: {face: ['Monaco', 'Consolas', 'monospaace'], size: 24, offset: {x: 0, y: 2}},
      target: '#termview',
    ...
    $('body').keydown(function(pEvent) {
      var a = null;
      if (!pEvent.ctrlKey && !pEvent.altKey && !pEvent.shiftKey) {
      ...
      else if (!pEvent.ctrlKey && !pEvent.altKey && !pEvent.shiftKey)
      {
        if (a)
          self._delegate.send({type: 'key', data: a});
      }
